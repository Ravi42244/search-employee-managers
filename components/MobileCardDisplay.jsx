      ...
      <div className=" flex  justify-start items-center ">
        {empData.error_message ? (
          ...
          >
              ...
              <h4 className="text-sm font-semibold">
                <Card className="px-2 py-1 mx-1  ">
                  <div className="w-full flex items-center justify-between">
                    ...
                    <div>
                      <div
                        className="group scale-50 cursor-pointer "
                        onClick={async () => {
                          if (navigator.clipboard) {
                            await navigator?.clipboard?.writeText(
                              empData?.reportTo[0]?.email
                            );
                          } else {
                            copy(empData?.email);
                          }
                      ...
                      >
                        <Badge
                          variant="outline"
                          className="group-hover:bg-neutral-200 transition duration-300 ease-in-out text-indigo-700"
                        >
                          <ClipboardCopy />
                        </Badge>
                      </div>
            ...
            {!empData?.reportTo ? (
              <div className="bg-white border rounded-md m-2 text-xs p-2">
                {empData?.firstName} {empData?.lastName}
              ...
              <Card className="px-2 py-1 m-2 mx-5">
                <div className="w-full flex items-center justify-between">
                  <div>
                    <CardTitle className="text-base mb-1">
                      {empData?.reportTo[0]?.firstName}{" "}
                      {empData?.reportTo[0]?.lastName}
                    </CardTitle>
                    <CardDescription className="text-xs">
                      {empData?.reportTo[0]?.email}
                    </CardDescription>
                    <CardDescription className="text-xs">
                      {empData?.reportTo[0]?.role}
                    </CardDescription>
                  </div>
                  <div>
                    <div
            ...
            {empData?.reportTo?.length > 1 && (
              <CollapsibleContent className="space-y-2">
                {empData?.reportTo.slice(1).map((empData, index) => (
                  <Card
                    key={empData?.email || index}
                    className="px-2 py-1 m-2 mx-5"
                  >
                    <div className="w-full flex items-center justify-between">
                      <div>
                        <CardTitle className="text-base mb-1">
